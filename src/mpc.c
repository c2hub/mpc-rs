...
char *mpc_err_string(mpc_err_t *x) {
  ...
  if (x->failure) {
    ...
    YELLOW "%s" RESET":" RED "error" RESET ": %s\n", x->filename, x->failure);
    return buffer;
  }

  mpc_err_string_cat(buffer, &pos, &max,
    YELLOW"%s"RESET":"GREEN"%i"RESET":"GREEN"%i"RESET":"RED "error" RESET": expected ", x->filename, x->state.row+1, x->state.col+1);
...
static void mpc_ast_print_depth(mpc_ast_t *a, int d, int index, FILE *fp) {
  ...
  if (strlen(a->contents)) {
    fprintf(fp, RED "%s" RESET ":" GREEN "%lu" RESET ":" YELLOW "%lu" RESET " '%s'"CYAN "[%d]" RESET "\n", a->tag,
      (long unsigned int)(a->state.row+1),
      (long unsigned int)(a->state.col+1),
      a->contents, index);
  } else {
    fprintf(fp, MAGENTA "%s" CYAN "[%d]" RESET " \n", a->tag, index);
  }

  for (i = 0; i < a->children_num; i++) {
    mpc_ast_print_depth(a->children[i], d+1, i, fp);
  }

}

void mpc_ast_print(mpc_ast_t *a) {
  mpc_ast_print_depth(a, 0, 0, stdout);
}

void mpc_ast_print_to(mpc_ast_t *a, FILE *fp) {
  mpc_ast_print_depth(a, 0, 0, fp);
}
